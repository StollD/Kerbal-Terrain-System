    ...
    public class TerrainDeformation : ScenarioModule
    {
        ...
        public override void OnLoad(ConfigNode node)
        {
            ...
            foreach (ConfigNode bodyNode in node.nodes)
            {
                ...
                {
                    Vector3d position = ConfigNode.ParseVector3D(deformationNode.GetValue("position"));
                    Vector3d velocity = ConfigNode.ParseVector3D(deformationNode.GetValue("velocity"));
                    Double force = Double.Parse(deformationNode.GetValue("force"));
                    Deformation deformation = new Deformation(position, body, velocity, force);
        ...
        public override void OnSave(ConfigNode node)
        {
            ...
            foreach (CelestialBody body in PSystemManager.Instance.localBodies.Where(b => b.pqsController != null))
            {
                ...
                foreach (Deformation deformation in pqsDeformation.deformations)
                {
                    ...
                    deformationNode.AddValue("velocity", ConfigNode.WriteVector(deformation.velocity));
                    deformationNode.AddValue("force", deformation.force);
                }
            }
        }
    }
